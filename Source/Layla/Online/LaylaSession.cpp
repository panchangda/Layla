...
ALaylaSession::ALaylaSession(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		OnCreateSessionCompleteDelegate = FOnCreateSessionCompleteDelegate::CreateUObject(this, &ALaylaSession::OnCreateSessionComplete);
		OnDestroySessionCompleteDelegate = FOnDestroySessionCompleteDelegate::CreateUObject(this, &ALaylaSession::OnDestroySessionComplete);

		OnFindSessionsCompleteDelegate = FOnFindSessionsCompleteDelegate::CreateUObject(this, &ALaylaSession::OnFindSessionsComplete);
		OnJoinSessionCompleteDelegate = FOnJoinSessionCompleteDelegate::CreateUObject(this, &ALaylaSession::OnJoinSessionComplete);

		OnStartSessionCompleteDelegate = FOnStartSessionCompleteDelegate::CreateUObject(this, &ALaylaSession::OnStartOnlineGameComplete);
	}
}
 ...
 */
void ALaylaSession::OnStartOnlineGameComplete(FName InSessionName, bool bWasSuccessful)
{
...
void ALaylaSession::HandleMatchHasStarted()
{
 ...
 *
 */
void ALaylaSession::HandleMatchHasEnded()
{
...
bool ALaylaSession::IsBusy() const
{ 
...
EOnlineAsyncTaskState::Type ALaylaSession::GetSearchResultStatus(int32& SearchResultIdx, int32& NumSearchResults)
{
	SearchResultIdx = 0;
	NumSearchResults = 0;
 ...
 */
const TArray<FOnlineSessionSearchResult> & ALaylaSession::GetSearchResults() const
{ 
 ...
 * @param bWasSuccessful true if the async action completed without error, false if there was an error
 */
void ALaylaSession::OnCreateSessionComplete(FName InSessionName, bool bWasSuccessful)
{
 ...
 * @param bWasSuccessful true if the async action completed without error, false if there was an error
 */
void ALaylaSession::OnDestroySessionComplete(FName InSessionName, bool bWasSuccessful)
{
...
bool ALaylaSession::HostSession(TSharedPtr<const FUniqueNetId> UserId, FName InSessionName, const FString& GameType, const FString& MapName, bool bIsLAN, bool bIsPresence, int32 MaxNumPlayers)
{
	...
	if (OnlineSub)
	{
		...
		{
			HostSettings = MakeShareable(new FLaylaOnlineSessionSettings(bIsLAN, bIsPresence, MaxPlayers));
...
bool ALaylaSession::HostSession(const TSharedPtr<const FUniqueNetId> UserId, const FName InSessionName, const FOnlineSessionSettings& SessionSettings)
{
	bool bResult = false;
...
void ALaylaSession::OnFindSessionsComplete(bool bWasSuccessful)
{
...
void ALaylaSession::ResetBestSessionVars()
{
	CurrentSessionParams.BestSessionIdx = -1;
}

void ALaylaSession::ChooseBestSession()
{
	// Start searching from where we left off
	for (int32 SessionIndex = CurrentSessionParams.BestSessionIdx+1; SessionIndex < SearchSettings->SearchResults.Num();)
	{
...
void ALaylaSession::StartMatchmaking()
{
	ResetBestSessionVars();
	ContinueMatchmaking();
}

void ALaylaSession::ContinueMatchmaking()
{	
	ChooseBestSession();
...
void ALaylaSession::OnNoMatchesAvailable()
{
	UE_LOG(LogOnlineGame, Verbose, TEXT("Matchmaking complete, no sessions available."));
	SearchSettings = NULL;
}

void ALaylaSession::FindSessions(TSharedPtr<const FUniqueNetId> UserId, FName InSessionName, bool bIsLAN, bool bIsPresence)
{
	IOnlineSubsystem* OnlineSub = Online::GetSubsystem(GetWorld());
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid() && CurrentSessionParams.UserId.IsValid())
		{
			SearchSettings = MakeShareable(new FLaylaOnlineSearchSettings(bIsLAN, bIsPresence));
...
bool ALaylaSession::JoinSession(TSharedPtr<const FUniqueNetId> UserId, FName InSessionName, int32 SessionIndexInSearchResults)
{
	bool bResult = false;
...
bool ALaylaSession::JoinSession(TSharedPtr<const FUniqueNetId> UserId, FName InSessionName, const FOnlineSessionSearchResult& SearchResult)
{
	bool bResult = false;
 ...
 * @param bWasSuccessful true if the async action completed without error, false if there was an error
 */
void ALaylaSession::OnJoinSessionComplete(FName InSessionName, EOnJoinSessionCompleteResult::Type Result)
{
...
bool ALaylaSession::TravelToSession(int32 ControllerId, FName InSessionName)
{
	IOnlineSubsystem* OnlineSub = Online::GetSubsystem(GetWorld());
	if (OnlineSub)
	{
...
void ALaylaSession::RegisterServer()
{
	IOnlineSubsystem* OnlineSub = Online::GetSubsystem(GetWorld());
	if (OnlineSub)
	{
		...
		{
			TSharedPtr<class FLaylaOnlineSessionSettings> LaylaHostSettings = MakeShareable(new FLaylaOnlineSessionSettings(false, false, 16));
			LaylaHostSettings->Set(SETTING_MATCHING_HOPPER, FString("TeamDeathmatch"), EOnlineDataAdvertisementType::DontAdvertise);
			LaylaHostSettings->Set(SETTING_MATCHING_TIMEOUT, 120.0f, EOnlineDataAdvertisementType::ViaOnlineService);
			LaylaHostSettings->Set(SETTING_SESSION_TEMPLATE_NAME, FString("GameSession"), EOnlineDataAdvertisementType::DontAdvertise);
			LaylaHostSettings->Set(SETTING_GAMEMODE, FString("TeamDeathmatch"), EOnlineDataAdvertisementType::ViaOnlineService);
			LaylaHostSettings->Set(SETTING_MAPNAME, GetWorld()->GetMapName(), EOnlineDataAdvertisementType::ViaOnlineService);
			LaylaHostSettings->bAllowInvites = true;
			LaylaHostSettings->bIsDedicated = true;
			if (FParse::Param(FCommandLine::Get(), TEXT("forcelan")))
			{
				UE_LOG(LogOnlineGame, Log, TEXT("Registering server as a LAN server"));
				LaylaHostSettings->bIsLANMatch = true;
			}
			HostSettings = LaylaHostSettings;
